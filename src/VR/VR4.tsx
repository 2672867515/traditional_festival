...
export const Vr = (): ReactElement => {
  ...
  const initBaseFactor = () => {
    ...
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    spotLight.position.set(80, 130, -130); //聚光灯位置
    spotLight.castShadow = true;//可以产生投影
    spotLight.angle = Math.PI / 4;//光锥角度45度
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    spotLight.shadow.penumbra = 0.05//阴影柔化程度
    spotLight.shadow.mapSize.width = 1024;//阴影贴图的宽度
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    spotLight.shadow.mapSize.innerHeight = 1024;//阴影贴图的高度
    // 添加聚光灯
    scene.add(spotLight)
    camera.position.x = 0;
    camera.position.y = 0;
